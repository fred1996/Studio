var connected = false;
var RETRY_INTERVAL = 1 * 60 * 1000;//设置1分钟断线自动连接心跳包
var timeout = null;
var adminrole = 100;
var client = {
    config: {
        ...
        dataTemplate: {
            ...
            'ADMINMSG_TEMP': '<div class="msgInfo"   ChartId="#ChartId#" ><span class="sendTime">#SENDTIME#</span><span style="color:red;font-weight:bold;margin-right:10px;margin-left:8px;display: inline-block;vertical-align: middle;">#ROLENAME#</span><span class="sayingMan tooltipstered"  uid="#USERID#">#USERNAME# :</span><div></div><span id="sayingInfo" style="margin: 5px 0;display: inline-block;color:#ff0000;font-size:18px  !important;font-weight:bold !important;line-height:21px;padding:6px;">#MSG#</span></div>',
            'ADMINMSGOnLine_TEMP_to': '<div class="msgInfo"  ChartId="#ChartId#" ><span class="sendTime">#SENDTIME#</span><span class="userRole" roleid="#ROLEID#">#ROLENAME#</span><span class="sayingMan tooltipstered" style="color:#fff;"  uid="#USERID#">#USERNAME#</span><span style=\"margin-right:5px;background: none repeat scroll 0 0 #eee;border-radius: 3px 3px 3px 3px;color: #333;line-height: 24px;padding:4px 6px;margin-right:5px; \">对</span><span class="toSayingMan tooltipstered" style="color:#fff;"  uid="#TOUSERID#">#TOUSERNAME#</span><span id="sayingInfo" style="margin: 5px 0;display: inline-block;color:#ff0000;font-size:18px  !important;font-weight:bold !important;line-height:inherit;padding:6px;">#MSG#</span></div>',
            'ADMINMSGOnLine_TEMP': '<div class="msgInfo"  ChartId="#ChartId#" ><span class="sendTime">#SENDTIME#</span><span style="color:red;font-weight:bold;margin-right:10px;margin-left:8px;display: inline-block;vertical-align: middle;">#ROLENAME#</span><span class="sayingMan tooltipstered"  uid="#USERID#">#USERNAME# :</span><span id="sayingInfo" style="margin: 5px 0;display: inline-block;color:#ff0000;font-size:18px  !important;font-weight:bold !important;line-height:inherit;padding:6px;">#MSG#</span></div>',
            'ONLINE_TEMP': '<div class="systemInfo"  ChartId="#ChartId#" ><span class="sendTime">#SENDTIME#</span>系统消息：<span class="sayingMan">#USERNAME#</span>上线了！</div>',
            'OFFLINE_TEMP': '<div class="systemInfo"  ChartId="#ChartId#" ><span class="sendTime">#SENDTIME#</span>系统消息：<span class="sayingMan">#USERNAME#</span>下线了！</div>',
    ...
    methods: {
        ...
        offline: function (jobj) {//登出时断开socket连接
            //var p = client.methods.initMsgHtml("2", jobj);
            //client.config.content().append(p);
            //if (homeMain.isscroll) {
            //    $(".nano").nanoScroller();
            //    $(".nano").nanoScroller({ scroll: 'bottom' });
        ...
        //correctionOnlineuserCount: function (socks) {
        //    if (socks) {
        //        $.ajax({ url: '/Home/CorrectionOnlineUserCount', data: { socketids: socks } }).done(function (result) {
        //        });
        //    }
        //},
                ...
                switch (mtype.toString()) {
                    ...
                    case "3":
                        ...
                        if (!jobj.rolename || jobj.rolename == "undefined") {
                            p = p.replace(/#ROLENAME#/g, "<span style='background: none repeat scroll 0 0 #d4371e;'  class=\"userRole\">游客</span>");
                        } else {
                            p = p.replace(/#ROLENAME#/g, client.replaceRoleImg(jobj.roleid));
                        }
                        p = p.replace(/#SENDTIME#/g, ctime);
                        p = p.replace(/#MSG#/g, client.displayMessage(jobj.msg, jobj.postfile));
                    ...
                    case "5":
                        ...
                        break;
                    case "6":
                        p = client.config.dataTemplate.MSGGiFtINFO_TEMP;
                        //if (jobj.roleid == 0 || !jobj.rolename || jobj.rolename == "undefined") {
                        //    p = p.replace(/#ROLENAME#/g, "<span  style='background: none repeat scroll 0 0 #d4371e;' class=\"userRole\">游客</span>");
                        //    p = p.replace(/#ROLEID#/g, "0");
                        //} else {
        ...
        toSay: function (jobj) {
            ...
            homeMain.BindTipEvent($("#MsgListWrapper .msgInfo[ChartId='" + jobj.ChatID + "']").find(".sayingMan"));
            $("#MsgListWrapper .msgInfo[ChartId='" + jobj.ChatID + "']").find(".sayingMan").click(function () {
                $(this).tooltipster("show");
            });
            homeMain.DisposeOldMsgs();
        },
        ...
        checkedMsg: function (jsParams) {
            $.ajax({ url: '/Home/CheckMsgitem', data: { chartId: jsParams } }).done(function (result) {
                ...
                $("#Msg .RemoveMsg[dataval='" + jsParams + "']").hide();
                if (doms) {
        ...
        sendGift: function (username,tousername,count) {
            var msg = "";
            if (count>=100) {
                msg = username+" 送 " + tousername + "[zslogo][zslogo]" + "X" + count;
            } else {
                msg = username+" 送 " + tousername + "[zslogo]" + "X" + count;
            }                                
            var msgdata = {
                ChatID: 0,
                roomid: homeMain.OnlineData.RoomId(),
            ...
            var p = client.methods.initMsgHtml(6, msgdata);
            client.config.content().append(p);
            $(".nano").nanoScroller();
            $(".nano").nanoScroller({ scroll: 'bottom' });
            client.socket.emit("sendUserGiftEvent", msgdata);
        },
        sendMsg: function () {
            ...
            client.config.content().append(p);
            homeMain.BindTipEvent($("#MsgListWrapper .msgInfo:last").find(".sayingMan"));
            $("#MsgListWrapper .msgInfo:last").find(".sayingMan").bind("click", function () {
                $(this).tooltipster("show");
            });
            if (homeMain.isscroll) {
                $(".nano").nanoScroller();
            ...
            //        $("#SayingInputVal").text(err.responseText);
            //    }
            //    return false;
            //});
            $.ajax({
                url: '/Home/CachingMsgList',
                type: 'POST',
                dataType: 'JSON',
                data: { input: sendMsg },
                error: function (err) {
                ...
                success: function (result) {
                    $("#txtPostFile").removeAttr("disabled");
                    if (result == 0) {//是否在黑名单
                        alert("你已被加入黑名单，不能发送消息!!");
                        return false;
                    }
                    if (result) {
                        msgdata.ChatID = result;
        ...
        retryConnectOnFailure: function (retryInMilliseconds) {
            ...
            timeout = setInterval(function () {
                if (!connected || !client.socket.connected) {
                    connected = true;
    ...
    events: {
        connectionEvent: function () {
            ...
            client.socket.on('connection', function (data) {
                ...
                //homeMain.UserOnLine();
                if (parseInt(homeMain.OnlineData.RoomId()) <= 0) {
                    homeMain.OnlineData.RoomId($("#hidRoomID").val());
                }
                client.socket.emit('onlineEvent', {
                    roomid: homeMain.OnlineData.RoomId(),
                    uid: client.config.uid(),
        ...
        showUserGiftEvent: function () {
            client.socket.on("sendUserGiftEvent", function (data) {
                var p = client.methods.initMsgHtml(6, data);
                client.config.content().append(p);
                $(".nano").nanoScroller();
                $(".nano").nanoScroller({ scroll: 'bottom' });
                homeMain.DisposeOldMsgs();
...
$(function () {
    ...
    setTimeout(function () {
        if (client.socket == null) {
            //client.socket = io.connect('http://172.16.15.24:1234');
        ...
        client.events.showUserGiftEvent();
