...
; (function ($) {
    ...
    var Danmu = function (element, options) {
        ...
        this.$element.css({
            ...
            //" text-shadow": "3px 0px 0px #E06BA6,-3px 0px 0px #E06BA6,0px 3px 0px #E06BA6,0px -3px 0px #E06BA6,6px 6px 10px #E06BA6",
            //"-webkit-text-fill-color": "white",
            //"  -webkit-text-stroke": "2px #E06BA6"
        });
                ...
                if ($(element).data("danmu_array")[$(element).data("nowtime")]) {
                    ...
                    for (var i = 0; i < danmus.length; i++) {
                        ...
                        if (danmus[i].position == 0) {
                            ...
                            if (danmus[i].text.length > 80 && danmus[i].text.length < 100) {
                                $('#' + fly_tmp_name).animate({ left: -$(this).width() * 4 }, options.speed+10000
                                    , function () { $(this).remove(); }
                                 );
                            } else if (danmus[i].text.length <=80) {
                                $('#' + fly_tmp_name).animate({ left: -$(this).width() * 3 }, options.speed
                                    , function () { $(this).remove(); }
                                 );   
                            }
                            else if (danmus[i].text.length >100) {
                                $('#' + fly_tmp_name).animate({ left: -$(this).width() *8+$("body").width()}, options.speed+40000
                                    , function () { $(this).remove(); }
                                 );
                            }
                        }
